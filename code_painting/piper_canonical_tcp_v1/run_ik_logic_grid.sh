...
while (($#)); do
  case "$1" in
    ...
    -h|--help) echo "Usage: $0 --task TASK --id ID [--arm auto|left|right] [--gpu N] [--output-root PATH] [--dry-run]"; exit 0 ;;
...
run_cell() {
  ...
  local cmd=(bash "$runner" --task "$TASK" --id "$ID" --ik-logic "$logic" --gpu "$GPU" --output-root "$root")
...
"$PY" "$SCRIPT_DIR/audit_ik_logic_inputs.py" \
  ...
  --task "$TASK" --id "$ID" --output "$AUDIT_OUT"
"$PY" "$SCRIPT_DIR/compose_ik_logic_grid.py" \
  --legacy-root "$LEGACY_ROOT" --canonical-root "$CANONICAL_ROOT" \
  --task "$TASK" --id "$ID" --input-audit "$AUDIT_OUT" --output "$VIDEO_OUT"
"$PY" "$SCRIPT_DIR/vscode_video.py" --root "$EP_OUT" --apply --workers 2 --manifest "$EP_OUT/vscode_transcode_manifest.json"
