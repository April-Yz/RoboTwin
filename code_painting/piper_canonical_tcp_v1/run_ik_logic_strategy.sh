...
TASK=""; ID=""; ARM=auto; STRATEGY=""; IK_LOGIC=""; GPU=0; OUTPUT_ROOT=""; CAMERA_PROFILE=d435; DRY_RUN=0

usage() {
  echo "Usage: $0 --task TASK --id ID --strategy orientation|fused|top_score --ik-logic legacy|canonical --output-root PATH [--camera-profile d435|wide] [--arm auto|left|right] [--gpu N] [--dry-run]"
}
...
[[ -n "$TASK" && -n "$ID" && -n "$STRATEGY" && -n "$IK_LOGIC" && -n "$OUTPUT_ROOT" ]] || { usage >&2; exit 2; }
...
LEFT_OBJ=""; RIGHT_OBJ=""; MESH_ARGS=()
case "$TASK" in
  pick_diverse_bottles) LEFT_OBJ=left_bottle; RIGHT_OBJ=right_bottle; MESH_ARGS=(--object_mesh_override left_bottle=/home/zaijia001/ssd/data/R1/hand/obj_mesh/cola/cola.obj --object_mesh_override right_bottle=/home/zaijia001/ssd/data/R1/hand/obj_mesh/bottle/bottle.obj) ;;
  place_bread_basket) LEFT_OBJ=basket; RIGHT_OBJ=bread; MESH_ARGS=(--object_mesh_override basket=/home/zaijia001/ssd/data/R1/hand/obj_mesh/basket/basket.obj --object_mesh_override bread=/home/zaijia001/ssd/data/R1/hand/obj_mesh/bread_y/bread_y.obj) ;;
  ...
  handover_bottle) LEFT_OBJ=right_bottle; RIGHT_OBJ=right_bottle; MESH_ARGS=(--object_mesh_override right_bottle=/home/zaijia001/ssd/data/R1/hand/obj_mesh/bottle/bottle.obj) ;;
  pnp_bread) LEFT_OBJ=left_bread; RIGHT_OBJ=right_bread; MESH_ARGS=(--object_mesh_override left_bread=/home/zaijia001/ssd/data/R1/hand/obj_mesh/bread_nj/bread_niujiao.obj --object_mesh_override right_bread=/home/zaijia001/ssd/data/R1/hand/obj_mesh/bread_yr/bread_yerong.obj) ;;
  pnp_tray) LEFT_OBJ=left_dark_red_cup; RIGHT_OBJ=right_bottle; MESH_ARGS=(--object_mesh_override left_dark_red_cup=/home/zaijia001/ssd/data/R1/hand/obj_mesh/dark_red_cup/dark_red_cup.obj --object_mesh_override right_bottle=/home/zaijia001/ssd/data/R1/hand/obj_mesh/bottle/bottle.obj) ;;
...
CMD=(
  env CUDA_VISIBLE_DEVICES="$GPU" "$PY" -u "$PLANNER"
  --anygrasp_dir "$ANY" --replay_dir "$REPLAY" --hand_npz "$HAND" --output_dir "$OUT"
...
printf '[run] ik_logic=%s strategy=%s camera=%s task=%s id=%s output=%s\n' "$IK_LOGIC" "$STRATEGY" "$CAMERA_PROFILE" "$TASK" "$ID" "$OUT"
...
if "${CMD[@]}" > >(tee "$OUT/stdout.log") 2> >(tee "$OUT/stderr.log" >&2); then
  ...
  echo "[failed] ik_logic=$IK_LOGIC strategy=$STRATEGY status=$status output=$OUT" >&2
  exit "$status"
fi
