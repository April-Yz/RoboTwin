...
def add_axis_marker(scene, pose, name, length=0.10, thickness=0.006, origin_color=(1.0, 1.0, 1.0)):
    """在场景中添加 RGB 坐标轴标记。

    Parameters
    ----------
    origin_color : tuple
        原点标记 (白色小方块) 的颜色，用于区分不同类别的坐标轴。
        图例见终端输出 AXIS_LEGEND。
    """
    ...
    add_visual_box(scene, sapien.Pose(pos), [thickness * 1.5] * 3, origin_color, f"{name}_origin")
...
AXIS_LEGEND = """
╔══════════════════════════════════════════════════════════════════╗
║              Viewer 坐标轴颜色图例 (Axis Legend)                  ║
╠══════════════════════════════════════════════════════════════════╣
║  原点色块         名称前缀              含义                      ║
║──────────────────────────────────────────────────────────────────║
║  ■ white          bottle_center         瓶子几何中心              ║
║  ■ yellow         place_target          放置目标位姿              ║
║  ■ cyan           ee_wrist              当前 link6 腕部 (joint6)  ║
║  ■ magenta        ee_tip                当前 夹爪尖端 (腕部+10cm) ║
║  ■ blue           stage_pregrasp        预抓取阶段 目标           ║
║  ■ green          stage_grasp_lower     下降抓取阶段 目标         ║
║  ■ gold           stage_lift            抬升阶段 目标             ║
║  ■ coral          stage_move_out        移出阶段 目标             ║
╠══════════════════════════════════════════════════════════════════╣
║  每个阶段有两种子类型：                                           ║
║    _wrist → link6 腕部 FK 直接结果 (原点色较亮)                  ║
║    _tip   → 腕部 +10cm 沿前进轴偏移 (原点色较暗)                 ║
╠══════════════════════════════════════════════════════════════════╣
║  如何找轴：                                                      ║
║  1. 先看原点色块颜色确定类别                                     ║
║  2. 红/绿/蓝 轴 = 局部 +X/+Y/+Z                                  ║
║  3. Z 轴 (蓝色) = 夹爪前进方向                                   ║
║  4. 对比 _wrist vs _tip 可验证 10cm 偏移是否匹配 URDF 指尖       ║
╚══════════════════════════════════════════════════════════════════╝
"""


def add_scene_debug_axes(task):
    """添加所有 debug 坐标轴，按类别使用不同原点色。"""
    print(AXIS_LEGEND)

    # ── 瓶子中心：白色原点 ──
    if hasattr(task, "bottle1"):
        add_axis_marker(task.scene, sapien.Pose(task.bottle1.get_pose().p),
                        "bottle_center_left", length=0.07, origin_color=(1.0, 1.0, 1.0))
    if hasattr(task, "bottle2"):
        add_axis_marker(task.scene, sapien.Pose(task.bottle2.get_pose().p),
                        "bottle_center_right", length=0.07, origin_color=(1.0, 1.0, 1.0))

    # ── 放置目标：黄色原点 ──
    if hasattr(task, "left_target_pose"):
        add_axis_marker(task.scene, pose_from_list(task.left_target_pose),
                        "place_target_left", length=0.10, origin_color=(1.0, 1.0, 0.0))
    if hasattr(task, "right_target_pose"):
        add_axis_marker(task.scene, pose_from_list(task.right_target_pose),
                        "place_target_right", length=0.10, origin_color=(1.0, 1.0, 0.0))

    # ── EE 当前位姿 + 阶段目标（由 task 提供，含 origin_color）──
