...
Use `run_ik_logic_grid.sh` to compare the same AnyGrasp/Human semantic source through original Legacy/OursV2 input adaptation and Canonical RTCP adaptation. V2 supports two internally consistent 2x4 profiles: `d435` (640x480/42.50deg/5 fps) and `wide` (640x360/90deg/10 fps). It audits all-eight camera parameters, source positions, axes, per-row target contracts, and the Canonical 19 cm link6 inverse. `run_ik_semantic_camera_batch.sh` produces six tasks x one episode x two profiles as flat `vis/<task>_id<id>_v<profile>.mp4` files. The old same-numeric-`T_W_RTCP` V1 under `outputs_ik_logic_grid_20260716` has invalid input semantics and is retained only as a counterexample. See `PIPER_IK_LOGIC_GRID_COMPARE.en.md` and `COMMANDS/piper_ik_logic_grid.en.md`.
