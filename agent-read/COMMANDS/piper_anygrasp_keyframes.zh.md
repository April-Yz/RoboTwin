...
新增原因：direct Piper hand replay 的 stored gripper frame 使用 `local +Z` 蓝轴作为 approach/forward；AnyGrasp raw candidate 的 wireframe 使用 `local +X` 红轴作为掌根到指尖的 finger-depth。候选选择阶段已经通过 `align_hand_rotation_to_candidate_convention()` 把 direct replay hand `local +Z` 对齐成 AnyGrasp candidate `local +X`，所以执行阶段必须保持 AnyGrasp/Piper 的 `identity + local-X` 坐标。
...
含义：让 robot target 跟 AnyGrasp gripper 可视化的 C 平面指尖方向一致。上一版 `swap_red_blue + local_z` 会把 target 蓝轴对齐到 C 形夹爪侧面法线，导致机器人朝向看起来和 gripper 可视化不一致。
...
- 2026-05-29 修正：`swap_red_blue + local_z` 被确认会把蓝轴转到 C 平面侧面法线；wrapper 改为 `identity + local_x`。
