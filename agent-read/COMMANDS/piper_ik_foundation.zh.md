...
- 相机以 `wrist_camera_pose_reference: urdf_end_link` 跟随仿真 `link6`，并使用 `wrist_camera_simulation_adapter: piper_pika_agx` 补真实 TCP/支架与 Pika CAD 的原点差及外壳净空。
- 仿真 adapter 不旋转标定光轴，也不翻转左右符号。右腕约 45 度 roll 在历史标定中持续存在，是实机安装差异，不是 0515 异常。
...
2026-06-11 使用 `foundation_input_0`：O.1/O.1.1/O.1.2 的 V1 viewer 与完整两阶段采集均成功；O.1.2 的 V2/V3/V4 完整采集也成功。input 0 的两个关键帧是 38/78。最终 wrist 适配的 V1 O.1.2 完整生成 validated replay、`episode0_succ.hdf5`、instructions 和八路视频；左右腕视频各 38 帧、320x240，抽帧确认两路均能看到各自瓶子和夹爪。
