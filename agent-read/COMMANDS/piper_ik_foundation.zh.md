...
新增任务 `pnp_tray_piper_ik_foundation`，复用 Foundation 基类，但左对象为 `left_dark_red_cup`、右对象为 `right_bottle`，关键帧来自 `code_painting/h2o_manual_review/pnp_tray/hand_keyframes_all.json`。O.2 默认 `foundation_action_target_source=object_keyframe`，action 使用 Foundation 第二关键帧 OBJ center，而不是 `h2_pure_d435` 的 EE xyz。动作顺序为 `pregrasp -> grasp -> close -> object-keyframe action -> open_gripper`。

pnp_tray 不沿用 pick_diverse 的 `standoff=0.14`；ID0 验证显示它会推偏左杯。O.2 默认和 wrapper 都使用 `foundation_grasp_standoff=0.105`。

2026-06-17 修正：旧 O.2 使用第二关键帧 EE target，ID0 为 `Y≈0.266`，导致 close 后 gripper 看起来向前走太远。新逻辑使用第二关键帧物体中心，ID0 action gripper target 改为 left `[0.0592,0.0749,0.8343]`、right `[0.1846,0.0690,0.8780]`，object error 约 `4.2cm/3.3cm`。
...
可选避障试验不改变默认命令：传 `--foundation_pregrasp_clearance_m 0.06` 会在 pregrasp 前插入抬高 waypoint。ID0 验证 `0.06m` 成功，`0.10m` 会使左杯旋转约 `16.3deg` 并失败。正式采集可用 `FOUNDATION_PREGRASP_CLEARANCE_M=0.06 bash collect_foundation_piper_ik_verified.sh pnp_tray v1 0 0 o2_pregrasp_clearance006`。
