...
- On March 16, 2026, this worktree was also used for a temporary `checkpoint_step_10000` quaternion-order smoke comparison (`legacy_xyzw` vs `robowin_wxyz`) from the separate `lingbot-va` repo. Both variants completed and produced `0/1` `place_can_basket` results, and the generated `results_posttrain_eval_step10000*` directories are now ignored here as transient debug artifacts.
