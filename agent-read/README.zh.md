...
- Foundation 批采集推荐使用独立 run tag、每 ID 一个 episode、最多三次 seed 和外层 timeout，避免复用旧 head-only HDF5 或无限重试。Wrist debug recorder 保存 VS Code 兼容的 H.264 左右/拼接 MP4 和参数 JSON；正式 wrapper 可用四个 `WRIST_*` 环境变量无 viewer 覆盖 tuning。
