...
- Viewer and collection use the same trajectory logic; collection adds pickle-schema validation and observation recording. The custom Piper IK execution loop must call `viewer.render()` after `_update_render()` for live main-window updates; move, settle, and gripper paths now follow that order. The motion viewer can concurrently show dynamic wrist/static head frustums and dual-wrist RGB. Wrist forward diagnosis distinguishes Pika physical/CAD `+X` from legacy debug `+Z` so frame-convention differences are not mistaken for 90-degree extrinsic errors; viewer yaw adjusts parent-frame `+Z` orientation, while roll only rotates the image about the optical axis.
...
The O.2 action order is `pregrasp -> grasp -> close -> action -> open_gripper`. By default, `action` comes from the second-keyframe OBJ center rather than the human EE xyz; the code preserves the gripper-to-object-center offset from grasp to generate the action gripper target. The pnp_tray left cup is shorter and smaller than the bottles; validation showed `foundation_grasp_standoff=0.14` pushes the cup on ID0, so O.2 defaults to `0.105`. Optional `foundation_pregrasp_clearance` inserts a lifted waypoint before pregrasp for avoidance trials, while default `0` keeps the no-avoidance path. Formal collection uses `collect_foundation_piper_ik_verified.sh pnp_tray ...` and still saves head and both wrist videos.
