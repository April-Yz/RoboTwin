...
O.2 的动作顺序为 `pregrasp -> grasp -> close -> action -> open_gripper`，其中默认 `action` 来源是第二关键帧 OBJ center，而不是人手 EE xyz；代码会保留 grasp 时 gripper 相对物体中心的偏移来生成 action gripper target。pnp_tray 的左杯比瓶子更矮更小，实测 `foundation_grasp_standoff=0.14` 会在 ID0 把杯子推偏；当前 O.2 默认使用 `0.105`。可选 `foundation_pregrasp_clearance` 会在 pregrasp 前插入抬高 waypoint 用于避障试验，默认 `0` 保持无避障路径。正式采集使用 `collect_foundation_piper_ik_verified.sh pnp_tray ...`，仍保存 head 和左右 wrist 视频。
