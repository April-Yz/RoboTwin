...
bash /home/zaijia001/ssd/RoboTwin/code_painting/run_plan_keyframes_human_replay_piper_d435.sh \
  --gpu 2 --ids 1 --viewer --tasks pick_diverse_bottles \
  --target_retreat_m 0.14 \
  --wrist_left_forward_offset_m -0.04 --wrist_right_forward_offset_m -0.02 \
...
## L16. Human Replay + Wrist 相机校准（2026-06-18）

human_replay 管线的 wrist 相机校准方案。标定数据来自 `calibration_bundle_piper_new_table_0515.json`，经 adapter + `cv_to_render` 轴转换后，相机 RED（光轴）= link6 +X。需 pitch=-90° 将光轴从 +X 转到 +Z（gripper 前进方向），再用 yaw/roll/lateral/forward 微调。

wrist 参数速查（所有任务共用，robot-specific）：

| 参数 | left | right | 含义 |
|---|---|---|---|
| `yaw_deg` | 0.182 | 0.840 | 消除光轴 Y 分量，使其落在 X-Z 平面 |
| `pitch_deg` | +90 | +90 | 光轴 +X → -Z，指向物体方向 |
| `roll_deg` | -14.635 | 44.649 | 画面横轴 ∥ 夹爪开合轴 (+Y) |
| `lateral_offset_m` | -0.0207 | +0.0274 | 相机居中于两指之间 |
| `forward_offset_m` | -0.04 | -0.02 | 沿光轴后退（pitch=+90 后光轴=-Z，负=向+Z后退=向前） |

### L16.1 pick_diverse_bottles（模式 A: 2全局关键帧）

**关键帧结构**：

```
G1 (frame ~46) → G2 (frame ~79)
  │                │
  pregrasp         action (xyz=G2, quat=G1 grasp)
  grasp
  close_gripper
```

**动作序列**：`pregrasp → grasp → close_gripper → action`（`--action_orientation_source grasp` 使 action 用第一关键帧朝向）。
...
**批量无 viewer（ID 0-101，GPU 2+3 分段）**：

输出到 `keyframe/L16_human_replay/pick_diverse_bottles/foundation_input_<ID>/`，含 `head_cam_plan.mp4` / `third_cam_plan.mp4` / `left_wrist_cam_plan.mp4` / `right_wrist_cam_plan.mp4`。
...
for id in $(seq 0 50); do
  timeout 600s bash /home/zaijia001/ssd/RoboTwin/code_painting/run_plan_keyframes_human_replay_piper_d435.sh \
    ...
    --wrist_left_pitch_deg 90 --wrist_right_pitch_deg 90 \
    --wrist_left_lateral_offset_m -0.0207 --wrist_right_lateral_offset_m 0.0274 \
    --output_root /home/zaijia001/ssd/RoboTwin/code_painting/anygrasp_plan_keyframes_piper_d435_replay/keyframe/L16_human_replay \
    || echo "FAIL L16 id=$id" >> /home/zaijia001/ssd/RoboTwin/code_painting/anygrasp_plan_keyframes_piper_d435_replay/keyframe/L16_failures.log
done
```
...
### L16.2 pnp_tray（模式 A: 2全局关键帧）

**关键帧结构**：同 L16.1，G1→G2，`pregrasp → grasp → close_gripper → action`。
...
### L16.3 place_bread_basket（模式 B: L2+R2 独立）

**关键帧结构**：

```
左手先: L1 (frame ~34) → L2 (frame ~64)
右手后: R1 (frame ~103) → R2 (frame ~119)
```

当前 planner 按全局 G1→G2 处理（取 `effective_keyframes[:2]`）。异步 L/R staged 执行参见 [L15.20 模式 B](#模式-b-异步执行设计l2r2-独立)。

**动作序列**：`pregrasp → grasp → close_gripper → action`。
...
### L16.4 stack_cups（模式 B: L2+R2 独立，右手先）
...
### L16.5 handover_bottle（模式 C: L1+R1+G1 交接）

**关键帧结构**：

```
R1 (右手拾取, frame ~39) → G (共享交接帧, frame ~80) → L1 (左手接收, frame ~103)
```

**动作序列**（当前按 G1→G2 处理）：`pregrasp → grasp → close_gripper → action`。

> **注意**：handover 场景中物体从右手传递到左手，双手在交接帧 G 处同时操作。当前 G1→G2 简化处理可能不适用，需后续单独适配。
...
bash /home/zaijia001/ssd/RoboTwin/code_painting/run_plan_keyframes_human_replay_piper_d435.sh \
  --gpu 2 --ids 0 --viewer --tasks handover_bottle \
  --trajectory_mode joint_interp --joint_trajectory_interpolation cubic \
  --ik_num_seeds 1 --ik_solution_selection joint_continuity \
  --ik_seed_perturbations 6 --ik_seed_perturbation_scale 0.05 \
  --ik_max_joint_step_rad 0 --execute_partial_cartesian_plan 0 \
...
### L16.6 pnp_bread（模式 D: G1+L1+R1 混合）

**关键帧结构**：

```
部分ID为模式 B (L2+R2 独立)，部分为模式 D:
G (共享帧, frame ~26) → L1 (frame ~52) → R1 (frame ~64)
```

**动作序列**（当前按 G1→G2 处理）：`pregrasp → grasp → close_gripper → action`。

> **注意**：不同 ID 可能属于不同模式（B 或 D），需按 `hand_keyframes_all.json` 动态判断。
...
bash /home/zaijia001/ssd/RoboTwin/code_painting/run_plan_keyframes_human_replay_piper_d435.sh \
  --gpu 2 --ids 0 --viewer --tasks pnp_bread \
  --trajectory_mode joint_interp --joint_trajectory_interpolation cubic \
  --ik_num_seeds 1 --ik_solution_selection joint_continuity \
  --ik_seed_perturbations 6 --ik_seed_perturbation_scale 0.05 \
  --ik_max_joint_step_rad 0 --execute_partial_cartesian_plan 0 \
