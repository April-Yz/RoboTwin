...
GPU_ID="${1:-${GPU_ID:-0,1,2,3}}"
CONFIG_NAME="${CONFIG_NAME:-pi0_v1_2_aloha_robotwin_full_distill}"
EXP_NAME="${EXP_NAME:-pi0_v1_2_run}"
...
printf 'Launching pi0 v1.2 full-ft training\n'
printf '  GPU_ID=%s\n' "${GPU_ID}"
printf '  CONFIG_NAME=%s\n' "${CONFIG_NAME}"
printf '  EXP_NAME=%s\n' "${EXP_NAME}"
printf '  BATCH_SIZE=%s\n' "${BATCH_SIZE}"
printf '  FSDP_DEVICES=%s\n' "${FSDP_DEVICES}"
printf '  EMA_DECAY=%s\n' "${EMA_DECAY}"
printf '  RUNNER=%s\n' "${RUNNER_LABEL}"
if [[ -n "${DATA_REPO_ID}" ]]; then
