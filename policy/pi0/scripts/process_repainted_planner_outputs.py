"""
Convert SAM-repainted planner head-cam videos plus planner wrist videos and
planner pose_debug.jsonl into the pi0 processed_data HDF5 intermediate format.

Typical usage:
python scripts/process_repainted_planner_outputs.py d_pour_blue "pour water" 27 \
...
def _extract_pose_field(record: dict[str, Any], key: str) -> np.ndarray:
    value = np.asarray(record[key], dtype=np.float64)
    if value.shape != (7,):
        raise ValueError(f"{key}: expected shape (7,), got {value.shape}")
    return value


def build_pose_sequence_from_pose_debug(pose_debug_jsonl: Path) -> np.ndarray:
    rows: list[np.ndarray] = []
    with pose_debug_jsonl.open("r", encoding="utf-8") as f:
        for line_idx, line in enumerate(f, start=1):
            line = line.strip()
            if not line:
                continue
            record = json.loads(line)
            left_pose = _extract_pose_field(record, "current_left_tcp_pose_world_wxyz")
            right_pose = _extract_pose_field(record, "current_right_tcp_pose_world_wxyz")
            left_state = np.concatenate(
                [
                    left_pose[:3],
                    quaternion_to_euler(left_pose[3:7]),
                    np.array([float(record["current_left_gripper_command"])], dtype=np.float64),
                ]
            )
            right_state = np.concatenate(
                [
                    right_pose[:3],
                    quaternion_to_euler(right_pose[3:7]),
                    np.array([float(record["current_right_gripper_command"])], dtype=np.float64),
                ]
            )
            state = np.concatenate([left_state, right_state], axis=0)
            if not np.isfinite(state).all():
                raise ValueError(f"Non-finite state detected at line {line_idx} in {pose_debug_jsonl}")
            rows.append(state.astype(np.float32))

    if not rows:
        raise RuntimeError(f"No records found in pose debug file: {pose_debug_jsonl}")
...
def process_dataset(
    ...
    for episode_id in candidate_ids:
        ...
        missing = [
            path
            for path in [head_episode_dir, planner_episode_dir, head_video_path, left_wrist_path, right_wrist_path, pose_debug_path]
            if not path.exists()
        ]
        if missing:
        ...
        print(f"          repaint_head={head_video_path}")
        print(f"          planner_left_wrist={left_wrist_path}")
        print(f"          planner_right_wrist={right_wrist_path}")
        print(f"          planner_pose_debug={pose_debug_path}")

        state_action_seq = build_pose_sequence_from_pose_debug(pose_debug_path)
    ...
    if processed == 0:
        raise RuntimeError("No usable episodes were processed. Check planner/head directory templates and review filters.")
    return processed


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Process repaint head videos together with planner wrist videos and pose_debug.jsonl into pi0 HDF5 episodes."
    )
    ...
    parser.add_argument(
        "--head-dir-template",
        type=str,
        default="id_{id}_head_cam_arm_gripper_cup_bottle_pad_target",
        help="Episode directory naming template under --head-root.",
    )
    parser.add_argument(
        "--head-video-name",
        type=str,
        default="target_with_original_head_cam_plan.mp4",
        help="Repainted head-camera video file inside each episode directory.",
    )
    parser.add_argument("--planner-root", type=Path, required=True, help="Root containing planner output directories.")
    parser.add_argument(
        "--planner-dir-template",
        type=str,
        default="d_pour_blue_{id}",
        help="Episode directory naming template under --planner-root.",
    )
    parser.add_argument("--left-wrist-video-name", type=str, default="left_wrist_cam_plan.mp4")
    parser.add_argument("--right-wrist-video-name", type=str, default="right_wrist_cam_plan.mp4")
    parser.add_argument("--pose-debug-name", type=str, default="pose_debug.jsonl")
    parser.add_argument(
        "--ids",
        type=int,
        nargs="*",
        default=None,
        help="Optional explicit episode ids. If omitted, ids are discovered from --head-root or filtered from --review-json.",
    )
    parser.add_argument(
        "--review-json",
        type=Path,
