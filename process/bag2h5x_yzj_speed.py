...
python3 bag2h5x_yzj_speed.py /data3/zjyang/real_r1/home/pine/yzj/pour --batch --workers 8 --output /data3/zjyang/real_r1/home/pine/yzj/pour/h5

python3 bag2h5x_yzj_speed.py /projects/zaijia001/R1/pour/pour_0201 --batch --workers 8 --output /projects/zaijia001/R1/h5/pour/pour_0201
python3 bag2h5x_yzj_speed.py /projects/zaijia001/R1/pour/pour_0202 --batch --workers 8 --output /projects/zaijia001/R1/h5/pour/pour_0202
python3 bag2h5x_yzj_speed.py /projects/zaijia001/R1/pour/pour_0203 --batch --workers 8 --output /projects/zaijia001/R1/h5/pour/pour_0203
python3 bag2h5x_yzj_speed.py /projects/zaijia001/R1/pour/pour_0203_1 --batch --workers 8 --output /projects/zaijia001/R1/h5/pour/pour_0203_1
...
def bag_to_h5(bag_path, output_path, num_workers=4):
    ...
    def interp_numerical(key):
        ...
        # 对于高频数据（>100Hz），使用最近邻插值降采样
        # 对于低频数据，使用线性插值
        freq = len(ts) / (ts[-1] - ts[0]) if len(ts) > 1 else 0
        
        if freq > 100:  # 高频数据用最近邻
            if vals.ndim == 1:
                f = interp1d(ts, vals, kind='nearest', fill_value="extrapolate")
            else:
                f = interp1d(ts, vals, axis=0, kind='nearest', bounds_error=False, fill_value=(vals[0], vals[-1]))
        else:  # 低频数据用线性插值
            if vals.ndim == 1:
